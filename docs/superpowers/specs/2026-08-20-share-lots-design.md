...
`get_recent_pnl` 用持仓的**最终**加权均价与**最终**股数对每个历史交易日做
mark-to-market（不维护逐笔分账本）。后果：某 code 若在后续某天被再次买入，
最近 5 天的每日金额曲线会把新的 (入场价, 股数) 回填到之前所有交易日，因此对
累积持仓而言，历史逐日序列是**近似**的。组合总收益（浮动 + 已实现）与收益率%
是精确的；仅逐日历史拆分近似。这源于决策记录 #3（累积到加权均价、不分账本）的
固有限制。
